module github.com/argoproj-labs/clusterprofile-integration-for-argocd
...
require sigs.k8s.io/cluster-inventory-api v0.1.3
...
replace (
	// The gitops-engine module has no tagged releases under the argo-cd/v3
	// module path, so a plain `require` can only ever resolve to a v0.0.0-*
	// pseudo-version, which loses to the higher v0.7.1-* base version that
	// argo-cd/v3's own go.mod declares for it. Force it to the exact
	// gitops-engine commit vendored by the pinned argo-cd/v3 release so the
	// build is reproducible without relying on upstream's internal
	// (local-path-only) replace directive.
