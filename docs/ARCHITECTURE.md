...
For custom access providers, provider resolution also accepts the deprecated
`status.credentialProviders` field for compatibility. When both fields contain
a provider with the same name, `status.accessProviders` takes precedence.
